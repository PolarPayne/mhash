...
uint32_t static sha1_rol32(uint32_t number, uint8_t bits) {
	return ((number << bits) | (number >> (32-bits)));
}

void static sha1_hashBlock(sha1nfo *s) {
	uint8_t i;
...
void static sha1_addUncounted(sha1nfo *s, uint8_t data) {
...
void static sha1_pad(sha1nfo *s) {
